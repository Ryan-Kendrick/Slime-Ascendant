  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-gray-900">
      <div className="flex w-full max-w-2xl flex-col items-center gap-8 px-8">
        ...
        {/* Progress Container - The "Receptacle" */}
        <div className="relative w-full max-w-xl">
          {/* Receptacle Background */}
          <div className="relative h-16 w-full overflow-hidden rounded-full border-4 border-green-500 bg-gray-700 shadow-inner">
            {/* Inner glow effect */}
            <div className="absolute inset-0 rounded-full bg-gradient-to-r from-transparent via-green-500/10 to-green-500/20"></div>

            {/* Slime Monster Progress Bar */}
            <div
              className="absolute left-0 top-0 flex h-full items-center transition-all duration-300 ease-out"
              style={{ width: `${Math.min(progressPercentage, 100)}%` }}>
              {/* Slime trail effect */}
              <div className="absolute inset-0 rounded-full bg-gradient-to-r from-green-600/60 to-green-400/80"></div>
            ...
            <div className="absolute right-2 top-1/2 -translate-y-1/2 transform">
              <div className="flex h-8 w-8 items-center justify-center rounded-full border-2 border-green-400 bg-gray-800">
          ...
          <div className="mt-4 flex justify-between text-sm text-gray-400">
            <span>Processing: {deltaProcessed.toLocaleString()}</span>
            <span>{Math.round(progressPercentage)}%</span>
            <span>Total: {delta.toLocaleString()}</span>
          </div>
        </div>
